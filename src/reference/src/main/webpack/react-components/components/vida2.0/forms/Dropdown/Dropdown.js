...
const Dropdown = ({
  ...
  // sorting down dropdown options on search
  ...
  const handleFocus = ({ keyCode }, isSearchBox) => {
    ...
    // to navigate through the dropdown options
    if (keyCode === 40) {
  ...
  return (
    <div
      className={
        errors && errors[name]
          ? "vida2-form-group form__group form__group--error"
          : "vida2-form-group form__group"
      }
    >
      {label.length > 0 && (
        <label
      ...
      <div className="form__dropdown-field">
        {iconClass && (
          <i
            className={
              `form__dropdown-pre-icon vida-dropdown-icon ${iconClass}` +
              `${isDisabled ? " disabled" : ""}`
            }
          ></i>
        )}
        ...
        <input
          ...
          type="text"
          autoComplete="off"
          className="vida-form-field-input form__dropdown-input"
          defaultValue={value}
          ref={searchRef}
