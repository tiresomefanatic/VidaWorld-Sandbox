      ...
      if (neededMarkers && neededMarkers.includes(markerValue)) {
        selectedLocationItem[markerValue].forEach((item) => {
          if (item.latitude && item.longitude) {
            ...
            } else if (markerValue === CONSTANT.CENTER.ATHER_CHARGING_STATION) {
              marker["details"] = {
                id: item.stationId,
                title: item.stationName,
                message: item.chargingStationAddress,
                image: icon.atherChargingStations
              };
            }
  ...
  useEffect(() => {
    try {
      if (selectedLocationItem) {
        ...
        switch (config.mode) {
          case CONSTANT.CENTER.DEFAULT /* Marker added for Experience center */:
            if (
            ...
            /* Marker added for Swapping station */
            /* if (
              selectedLocationItem.swappingStations &&
              selectedLocationItem.swappingStations.length
            ) {
              markers.push(...addMapMarkers(CONSTANT.CENTER.SWAPPING_STATION));
            } */

            break;
          case CONSTANT.CENTER
            .CHARGING_STATION /* Marker added for Charging station */:
            if (
              selectedLocationItem.chargingStations &&
              selectedLocationItem.chargingStations.length
            ) {
            ...
            if (
              selectedLocationItem.atherChargingStations &&
              selectedLocationItem.atherChargingStations.length
            ) {
              markers.push(
                ...addMapMarkers(CONSTANT.CENTER.ATHER_CHARGING_STATION)
              );
            }
          ...
          <ul className="vida-location-finder__locations">
            {displayList.map((location) => (
              <React.Fragment key={index}>
                {location.id !== "atherChargingStations" ? (
                  <li
                    key={location.id}
                    className="vida-location-finder__center"
                  >
                    <img
  ...
  const otherVariant = () => (
    <div className="vida-location-finder-centre">
      <div className="vida-location-finder-centre__center-details">
        ...
        />
        {/* <div className="vida-location-finder-centre__col">
          <div className="vida-location-finder-centre__field-selection">
            <div className="form__group">
              <div className="form__field-select-wrapper">
                <i className="icon-location-marker"></i>
                <select
