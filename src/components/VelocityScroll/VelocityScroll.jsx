...
function ParallaxText({ children, baseVelocity = 100, ...props }) {
  ...
  return (
    <div ref={containerRef} className="velocity-scroll__parallax-container" {...props}>
      <motion.div className="velocity-scroll__parallax-inner" style={{ x }}>
        {Array.from({ length: repetitions }).map((_, i) => (
          <span key={i} ref={i === 0 ? textRef : null} className="velocity-scroll__parallax-text">
            {children} {" "}
          </span>
        ))}
      </motion.div>
...
export function VelocityScroll({ defaultVelocity = 5, numRows = 2, children, className = "", ...props }) {
  return (
    <div className={`velocity-scroll__container ${className}`} {...props}>
